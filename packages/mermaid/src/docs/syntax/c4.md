...
Element text (name, type and description) wraps to fit the element width, which is set by the `c4.width` config value (default `216`). To keep text on one line, raise `c4.width` so the longest line fits:
