...
A `direction` statement sets the layout direction used by the unified
renderer: `TB` (default), `BT`, `LR` or `RL`. The legacy renderer keeps its
row-based grid and ignores the statement.
...
A `SHOW_LEGEND()` statement appends a legend below the diagram listing every
element type used (with its fill color) and every tag defined with
...
### Clickable links (v<MERMAID_RELEASE_VERSION>+)

The unified renderer turns elements and boundaries with a `$link` into
clickable links (with `securityLevel: 'sandbox'` the link opens in the top
window instead of a new tab). `$link` on a relationship is not supported by
the unified renderer.

```
C4Context
Person(customerA, "Banking Customer A", $link="https://github.com/mermaid-js")
```
